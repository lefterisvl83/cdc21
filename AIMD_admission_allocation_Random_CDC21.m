...
lambda = 100;               % mean value of arrival rate    ***!!!!    %
alfa = [1 2 3 4]'*5;        % alfa_i                                   %
veta = [1 1 1 1]'*.5;       % beta_i                                   %
sim_time = 100;             % continuous time (actual time)            %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
uk = []; % admission
uk(:,1) = uk0;
...
y(i) = exprnd(1/100); % interarrival times (according to exponential distribution)
end
% %
yy = [];
yy(1) = 0;
...
% 
% the remaining code is identical to the deterministic case, refer to
...
for i = 1:interv
    if abs(t_cont(i) - Tkcum(j)) > tol
        ...
        w_cont(:,i) = ceil(wk(:,j-1) + diag(veta)*uk(:,j-1).*(t_cont(i)-Tkcum(j-1))...
...
for i = 1:4
    plot(1:eventss, gkamak(i,1:eventss),'-', 'LineWidth', 1.5, 'color', C{i})
    legtxt{j} = ['node-', num2str(i)]; 
    hold on
    j = j + 1;
    plot( 1:(eventss-1), uk_av(i,1:(eventss-1)),'--', 'LineWidth', 1.5, 'color', C{i})
    legtxt{j} = ['node-', num2str(i)];
    j = j + 1;
    xlabel('Events')
